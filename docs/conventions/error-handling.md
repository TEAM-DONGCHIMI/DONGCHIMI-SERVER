...
커스텀 어노테이션 `@ApiErrorCode`(`api:core-api`)로 에러 응답을 문서화한다.
enum 클래스를 통째로 참조하지 않고 **실제로 그 API가 던지는 개별 코드만** 골라 문서화한다 — enum 전체를 펼치면 해당 API와 무관한 코드까지 응답 예시에 섞여 문서가 부정확해진다. `type`으로 `ErrorCode` enum 클래스를, `codes`로 그 안의 상수 이름을 넘긴다. 한 어노테이션에는 **같은 `type`의 코드만** 담으므로, 서로 다른 enum을 쓸 때는 어노테이션을 여러 번 붙인다(`@Repeatable`).
...
```kotlin
// 같은 enum의 코드는 codes에 나열해 한 줄로, 다른 enum은 어노테이션을 추가로 붙인다
...
- `codes`에 넘긴 이름은 런타임에 enum 상수와 대조해 검증한다. 존재하지 않는 코드 이름을 적으면 애플리케이션 기동 시점이 아니라 해당 문서를 조립할 때 `error(...)`로 즉시 실패하므로 오탈자를 잡을 수 있다.
- 개별 코드를 **컴파일 타임**에 타입 안전하게 강제하려면 `ErrorCode`를 `enum class` 대신 `object` 계층으로 바꿔야 해서(컨벤션 전면 변경) 배보다 배꼽이다. 문자열 이름 + 런타임 검증으로 절충한다.

`ApiErrorCodeCustomizer`(`OperationCustomizer` 구현체, `@Component`)가 어노테이션마다 `codes`를 실제 `ErrorCode` 상수로 resolve해 OpenAPI 응답에 추가한다. springdoc이 `OperationCustomizer` 타입 빈을 자동 감지하므로 별도 등록은 필요 없다.
...
class ApiErrorCodeCustomizer : OperationCustomizer {
    override fun customize(operation: Operation, handlerMethod: HandlerMethod): Operation {
        ...
        // 어노테이션마다 codes를 enum 상수로 resolve(없는 이름이면 error로 실패)
