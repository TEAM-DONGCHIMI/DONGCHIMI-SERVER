...
class FlyerControllerTest :
    FunSpec({
        ...
        test("오늘의 특가 전단 미리보기 조회 시 apiUser의 userId로 Facade를 호출하고 성공 응답을 반환한다") {
            val flyerService = Mockito.mock(FlyerService::class.java)
            val flyerPreviewQueryFacade = Mockito.mock(FlyerPreviewQueryFacade::class.java)
            val response = sampleDailyResponse()
            Mockito
                .`when`(
                    flyerPreviewQueryFacade.getDailyPreview(
                        ArgumentMatchers.eq(apiUser.userId),
                        ArgumentMatchers.eq(marketId),
                        anyLocalDateTime(),
                    ),
            ...
            val result = controller.getDailyPreview(apiUser, marketId)
...
private fun sampleDailyResponse() =
